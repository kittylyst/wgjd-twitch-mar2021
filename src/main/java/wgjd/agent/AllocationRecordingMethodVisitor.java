...
public final class AllocationRecordingMethodVisitor extends GeneratorAdapter {
  private final String runtimeAccounterTypeName = "wgjd/agent/RuntimeCostAccounter";

  public AllocationRecordingMethodVisitor(
      MethodVisitor methodVisitor, int access, String name, String desc) {
    super(Opcodes.ASM5, methodVisitor, access, name, desc);
  }

  /**
   * This method is called when visiting an opcode with a single int operand. For our purposes this
   * is a NEWARRAY opcode.
   *
   * @param opcode
   * @param operand
   */
  ...
  public void visitIntInsn(final int opcode, final int operand) {
    ...
    super.visitMethodInsn(
        Opcodes.INVOKESTATIC, runtimeAccounterTypeName, "recordArrayAllocation", "(II)V", true);
    super.visitIntInsn(opcode, operand);
  }

  /**
   * This method is called when visiting an opcode with a single operand, that is a type
   * (represented here as a String).
   *
   * <p>For our purposes this is either a NEW opcode or a ANEWARRAY
   *
   * @param opcode
   * @param type
   */
